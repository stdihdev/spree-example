module Nelou
  module Price
    module Sales
      ...
      included do
        has_many :sale_prices, class_name: 'Nelou::SalePrice'
      end

      # TODO: also accept a class reference for calculator type instead of only a string
      def put_on_sale(value, calculator_type = 'Nelou::Calculator::AmountSalePriceCalculator', start_at = Time.now, end_at = nil, enabled = true)
        new_sale(value, calculator_type, start_at, end_at, enabled).save
      end
      alias create_sale put_on_sale

      def new_sale(value, calculator_type = 'Nelou::Calculator::AmountSalePriceCalculator', start_at = Time.now, end_at = nil, enabled = true)
