Spree.ready ($) ->
  ...
  Spree.onPayment = () ->
    if ($ '#checkout_form_payment').is('*')
      ...
      ($ '#payment-method-fields').find('input[type=radio]').on 'click', ->
        ($ '#payment-method-fields label').removeClass 'active'
        if ($ this).is ':checked'
      ...
      if ($ '.existing_cards').is('*')
        ...
        ($ '.use_existing_card_yes').click ->
          $context = ($ this).parents('.payment-method-box')
          $context.find('.payment-method-fields').hide()
          $context.find('.existing-cc-radio').prop('disabled', false)
          $('.existing_cards').removeClass('mod-disabled')

        ($ '.use_existing_card_no').click ->
          $context = ($ this).parents('.payment-method-box')
          $context.find('.payment-method-fields').show()
          $context.find('.existing-cc-radio').prop('disabled', true)
          $('.existing_cards').addClass('mod-disabled')
